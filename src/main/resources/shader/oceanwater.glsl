...
void main() {
    ...
    vec3 oceanColor = mix(refractColor * vec3(0.7, 0.85, 1.0), vec3(0.0056, 0.0224, 0.056), 1.0 - normalDepth);
    ...
    if(cameraPos.y < waterLevel) {
        ...
        fresnel = 0.02 + 0.98 * pow(1.0 - dot(normalize(vec3(0.0, 1.0, 0.0) + normal * 0.2), view), 5.0);
    }

    // Diffuse
    float diffuse = clamp(dot(normal, normalize(light.direction.xyz)), 0.0, 1.0);

    vec3 color = diffuse * mix(oceanColor, skyColor, fresnel);
